...
{
    double rad = (a*b)/(len(a)*len(b));
    return acos( max(-1.0, min(1.0, rad))  );
}

///small angle between two vector
double smallAngle( PT a, PT b )
{
     double rad = acos( min(abs(a*b)/len(a)/len(b), 1.0) );
     return rad;
}
...
struct Plane
{
    ...
    PT n;
    Plane(){}
    Plane(PT p, PT q, PT r ) : Plane( (q-p)^(r-p), ((q-p)^(r-p))*(p) ) {}
    ///normal in direction of p,q,r
    Plane(double a, double b, double c, double d) : a(a), b(b), c(c), d(d), n(PT(a,b,c)){}
    Plane(PT n, double d) : n(n), a(n.x), b(n.y), c(n.z), d(d) {}
    Plane(const Plane &p) : n(p.n), d(p.d), a(p.a), b(p.b), c(p.c) {}
};


///returns 0 if t is on plane p
///returns 1 if t is on positive side of normal
///returns -1 if t is on negative side of normal
int side( Plane &p, PT a )
{
    double d = p.a*a.x + p.b*a.y + p.c*a.z - d;
    return dcmp(d);
}
...
///returns 1 if t is on positive side of a,b,c
///returns -1 if t is on negative side of a,b,c
...
{
    PT n = (b-a)^(c-a);
    PT dx = unitVector( b-a );
    PT dy = unitVector( n^(b-a) );
    vector<PT>newpt;
...
{
    PT d = p1.n^p2.n;
    PT u = ((p2.n*p1.d - p1.n*p2.d)^d)/(d*d);
    return Line(d, u);
}
...
///two planes are parallel if cross product of their normal is zero
///two planes are parallel if dot product of their normal is zero
...
struct Sphere
{
    ...
    {
        return PT( r*cos(phi)*cos(lamda),r*cos(phi)*sin(lamda), r*sin(phi) );
    }
};
...
{
    vector<PT>vec;
    double h2 = r*r - distancePointLine(l, cen)*distancePointLine(l, cen);
    if( dcmp(h2) < 0 )
        return vec;
    if( dcmp(h2) == 0 )
    {
...
/// let’s consider the case of a spherical triangle ABC. It’s area is given
///by r2(a + b + c - pi) where r is the radius of the sphere and
///a; b; c are the amplitudes of the three interior angles of ABC
...
double greatCirclePointDistance( Sphere s, double phi1, double lamda1, double phi2, double lamda2  )
{
...
double greatCircleArea( Sphere s, double phi1, double lamda1, double phi2, double lamda2, double phi3, double lamda3 )
{
    PT p1 = s.convert( phi1, lamda1 ), p2 = s.convert( phi2, lamda2 ), p3 = s.convert( phi3, lamda3 );
    double a = angleBetweenTwoPlane( Plane(s.cen, p1, p2), Plane(s.cen, p1, p3) );
    double b = angleBetweenTwoPlane( Plane(s.cen, p2, p3), Plane(s.cen, p2, p1) );
    double c = angleBetweenTwoPlane( Plane(s.cen, p3, p1), Plane(s.cen, p3, p2) );
